draw.triple.venn.selma <- function (vector1, vector2, vector3, category = rep("",3),
                                    rotation = 1, reverse = FALSE, euler.d = TRUE, scaled = TRUE, 
          lwd = rep(2, 3), lty = rep("solid", 3), col = rep("black", 3), 
          fill = NULL, shift, alpha = rep(0.5, 3), label.col = rep("black",7), 
          cex = rep(1, 7), fontface = rep("plain", 7), fontfamily = rep("serif",7), 
          cat.pos = c(-40, 40, 180), cat.dist = c(0.05, 0.05, 0.025), 
          cat.col = rep("black", 3), cat.cex = rep(1, 3), 
          ...
          direct.area = FALSE, area.vector = 0,  ...) 
{
  ...
  for (i in 1:7) {
    if (processedLabels[i]>0) {
    grob.list <- gList(grob.list, textGrob(label = processedLabels[i], 
                                           x = shift + cell.x[i], y = cell.y[i], gp = gpar(col = label.col[i], 
