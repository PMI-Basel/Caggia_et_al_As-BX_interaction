---
title: "Arsen - 01: Import & Normalization"
author: "Jan Waelchli"
geometry: margin=2cm
output:
  pdf_document:
...
WT maize plants and bx1 mutants with a W22 background were grown. Half of them were treated with 0 ppm arsen (CTRL) and the other half with 100 ppm arsen. We analyse the shift of the bacterial and fungal communities in the plant rhizosphere.
...
paths <- list(DESIGN = c("design_stats.xlsx"),
              bASV_table = c("../../4_output/bacteria_ASV/ASV100/bacteria_DAT100.tab"),
              btaxa = c("../../4_output/bacteria_ASV/ASV100/bacteria_TAXA100.tab"),
              btrack = c("../../4_output/bacteria_ASV/quality_check/bacteria_track.xlsx"),
              fASV_table = c("../../4_output/fungi_ASV/ASV100/fungi_DAT100.tab"),
              ftaxa = c("../../4_output/fungi_ASV/ASV100/fungi_TAXA100.tab"),
...
#arsen
DESIGN$arsen[DESIGN$arsen=="0ppm"] <- "CTRL"
DESIGN$arsen[DESIGN$arsen=="100ppm"] <- "arsen"

#factors
DESIGN$genotype <- factor(DESIGN$genotype, levels=c("WT", "bx1"))
DESIGN$arsen <- factor(DESIGN$arsen, levels=c("CTRL", "arsen"))
...
DESIGN$cols_arsen <- NA
DESIGN$cols_arsen[DESIGN$arsen=="CTRL"] <- "lemonchiffon3"
DESIGN$cols_arsen[DESIGN$arsen=="arsen"] <- "lemonchiffon4"
...
temp <- data.frame(DESIGN$arsen, DESIGN$cols_arsen)
temp <- plyr::ddply(temp, .variables="DESIGN.cols_arsen", .fun=unique)
level_cols_arsen <- as.character(temp[,2])
names(level_cols_arsen) <- temp[,1]
...
track$arsen <- as.factor(DESIGN$arsen)
track_long <- tidyr::pivot_longer(track, cols= !(c("FLD", "genotype", "arsen")), names_to = "steps", values_to = "reads")
...
track_bac <- ggplot(track_long, aes(steps, reads, col=genotype, lty=arsen, group=paste(genotype, arsen, sep="_")))+
...
track_fun <- ggplot(track_long, aes(steps, reads, col=genotype, lty=arsen, group=paste(genotype, arsen, sep="_")))+
                geom_jitter(alpha=0.1, width = 0.2)+
                geom_smooth(aes(), se=F, method="loess", formula="y~x")+
                theme_bw()+
                scale_color_manual(values = level_cols_genotype)+
...
## boxplot
df <- data.frame(col_sum=colSums(bDAT),colnames=colnames(bDAT), arsen=bDESIGN$arsen, 
...
seq_nr <- ggplot(data=df, aes(x=genotype, y=col_sum, fill=genotype)) + 
              ...
              facet_grid(.~ arsen, scales='free_x', space="free")+
              ggtitle("Bacteria: Sequencing depth")

## barplot
df <- df[order(df$col_sum, decreasing = T),]
df <- df[order(df$arsen),]
df$x <- c(1:sum(df$arsen == "CTRL"), 1:sum(df$arsen == "arsen"))
...
## boxplot
df <- data.frame(col_sum=colSums(fDAT),colnames=colnames(fDAT), arsen=fDESIGN$arsen, 
...
To decide on how to normalize the data we follow the recommendation of Weiss et al. (2017, Microbiome Journal) and inspect whether there are differences in sequencing depths between the different arsen-treatments and genotypes by using the non-parametric Kruskal-Wallis Test.

```{r seq numbers per group bacteria, echo=F, message=F, warning=F}

#BACTERIA
group <- as.factor(paste(bDESIGN$genotype, bDESIGN$arsen, sep="_"))
...
#FUNGI
group <- as.factor(paste(fDESIGN$genotype, fDESIGN$arsen, sep="_"))
...
detect_outliers <- function(DESIGN, DAT_norm, Arsen, species="bacteria"){

  #subset
  if(species=="bacteria"){samples_sub <- DESIGN$bac_FLD[DESIGN$arsen == Arsen]}
  if(species=="fungi"){samples_sub <- DESIGN$fun_FLD[DESIGN$arsen == Arsen]}
...
#get outliers
bac_outliers_CTRL <- detect_outliers(DESIGN=bDESIGN, DAT_norm=bDAT_norm, Arsen="CTRL", species="bacteria")
fun_outliers_CTRL <- detect_outliers(DESIGN=fDESIGN, DAT_norm=fDAT_norm, Arsen="CTRL", species="fungi")
bac_outliers_arsen <- detect_outliers(DESIGN=bDESIGN, DAT_norm=bDAT_norm, Arsen="arsen", species="bacteria")
fun_outliers_arsen <- detect_outliers(DESIGN=fDESIGN, DAT_norm=fDAT_norm, Arsen="arsen", species="fungi")


#output
outliers <- data.frame(Species=c("Bacteria","Fungi"),
                       "Arsen:CTRL"=c(length(bac_outliers_CTRL), length(fun_outliers_CTRL)),
                       "Arsen:arsen"=c(length(bac_outliers_arsen), length(fun_outliers_arsen)))
...
#bacteria
samples_to_remove_bac <- c(bac_outliers_CTRL, bac_outliers_arsen)
...
#fungi
samples_to_remove_fun <- c(fun_outliers_CTRL, fun_outliers_arsen)
...
table_bac <- table(bDESIGN$genotype, bDESIGN$arsen)
table_fun <- table(fDESIGN$genotype, fDESIGN$arsen)
...
#add arsen
brarefaction_long$arsen <- NA
for (i in 1:nrow(brarefaction_long)) {
  brarefaction_long$arsen[i] <- as.character(DESIGN$arsen[DESIGN$bac_FLD == brarefaction_long$species[i]])
}
...
bac_rareplot <- ggplot(brarefaction_long, aes(x = subsample, y = value, group = species)) +
                  theme_bw() +
                  geom_line(aes(color=genotype, lty=arsen))+
                  scale_color_manual(values = level_cols_genotype)+
...
#add arsen
frarefaction_long$arsen <- NA
for (i in 1:nrow(frarefaction_long)) {
  frarefaction_long$arsen[i] <- as.character(DESIGN$arsen[DESIGN$fun_FLD == frarefaction_long$species[i]])
}
...
#print
#png("figs_for_paper/fig3.png",6400, 4000, res = 600)
fig3
#dev.off()
...
saveRDS(level_cols_arsen, "level_cols_arsen.RDS")
