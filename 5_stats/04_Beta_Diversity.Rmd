...
-   **Q1: Has arsen changed the beta diversity?**
-   **Q2: Is beta diversity different between the genotypes?**
-   **Q3: Are there differences in beta diversity between the different genotypes after treating plants with arsen?**
...
bdist_paov <- vegan::adonis2(bdist ~ (genotype + arsen)^2, data=bDESIGN)
pander(bdist_paov, caption="Bacteria: PERMANOVA")

#FUNGI
fdist <- vegan::vegdist(t(fDAT), method="bray")
fdist_paov <- vegan::adonis2(fdist ~ (genotype + arsen)^2, data=fDESIGN)
...
### Figure 6.1 \| PCoA - genotype:arsen effect on beta diversity
...
```{r genotype arsen PCoA, echo=F, warning=F, eval=T, fig.height=9, fig.width=12}

#BACTERIA
bPCoA_bray <- phyloseq::ordinate(bPHYSEQ, method="PCoA", distance="bray")
p0 <- phyloseq::plot_ordination(bPHYSEQ, bPCoA_bray, color= "genotype", shape="arsen", axes = c(2,3))
blines <- centroid_lines(PHYSEQ = bPHYSEQ, dist_matrix = bPCoA_bray, group = c("genotype","arsen"), axes = c(2,3))
p0 <- p0 + geom_segment(aes(xend=blines$Axis.1, yend=blines$Axis.2), x = blines$Axis.1.means, y = blines$Axis.2.means, alpha=0.3)
...
p0 <- p0 + theme_bw()
p0 <- p0 + ggtitle("Bacteria - PCoA: Genotype:Arsen effect")
fig_bac <- p0


#FUNGI
fPCoA_bray <- phyloseq::ordinate(fPHYSEQ, method="PCoA", distance="bray")
p0 <- phyloseq::plot_ordination(fPHYSEQ, fPCoA_bray, color= "genotype", shape="arsen", axes = c(2,3))
flines <- centroid_lines(PHYSEQ = fPHYSEQ, dist_matrix = fPCoA_bray, group = c("genotype","arsen"), axes = c(2,3))
p0 <- p0 + geom_segment(aes(xend=flines$Axis.1, yend=flines$Axis.2), x = flines$Axis.1.means, y = flines$Axis.2.means, alpha=0.3)
p0 <- p0 + geom_point(size=3)
#p0 <- p0 + geom_text(aes(label=fPHYSEQ@sam_data$fun_FLD))
p0 <- p0 + scale_color_manual(values=level_cols_genotype)
p0 <- p0 + theme_bw()
p0 <- p0 + ggtitle("Fungi - PCoA: Genotype:Arsen effect")
fig_fun <- p0
...
### Figure 6.2 \| CAP - genotype:arsen effect on beta diversity

\vspace{5mm}

```{r CAP genotype arsen, echo=F, warning=F, eval=T, fig.height=9, fig.width=12}

#BACTERIA
bCAP_bray <- phyloseq::ordinate(bPHYSEQ, method="CAP", ~ genotype * arsen, distance="bray") #CAP
...
score <- paste("[ ~ genotype * arsen \n arsen: ",
               format(bdist_paov$R2[rownames(bdist_paov)=="arsen"] * 100, digits=2, nsmall=1),
               "% var, P=",format(bdist_paov$`Pr(>F)`[rownames(bdist_paov)=="arsen"], digits=2),"]",
                "\n genotype: ",
               ...
               "% var, P=",format(bdist_paov$`Pr(>F)`[rownames(bdist_paov)=="genotype"], digits=2),"]",
                "\n arsen:genotype: ",
               format(bdist_paov$R2[rownames(bdist_paov)=="genotype:arsen"] * 100, digits=2, nsmall=1),
               "% var, P=",format(bdist_paov$`Pr(>F)`[rownames(bdist_paov)=="genotype:arsen"], digits=2),"]", sep = "")
#plot
p0 <- phyloseq::plot_ordination(phy_sub, bCAP_bray, color="genotype", shape="arsen", title =  "Bacteria - CAP: Genotype:Arsen effect", axes=c(1,2))
p0$layers <- p0$layers[-1]
blines <- centroid_lines(phy_sub, bCAP_bray, c("genotype","arsen"), axes=c(1,2))
p0 <- p0 + geom_segment(aes(xend=blines$Axis.1, yend=blines$Axis.2), x = blines$Axis.1.means, y = blines$Axis.2.means, alpha=0.3)
p0 <- p0 + geom_point(size=3)
#p0 <- p0 + geom_text(aes(label=bPHYSEQ@sam_data$bac_FLD))
p0 <- p0 + scale_color_manual(values=level_cols_genotype)
...
#FUNGI
fCAP_bray <- phyloseq::ordinate(fPHYSEQ, method="CAP", ~ genotype * arsen, distance="bray") #CAP
...
score <- paste("[ ~ genotype * arsen \n arsen: ",
               format(fdist_paov$R2[rownames(fdist_paov)=="arsen"] * 100, digits=2, nsmall=1),
               "% var, P=",format(fdist_paov$`Pr(>F)`[rownames(fdist_paov)=="arsen"], digits=2),"]",
                "\n genotype: ",
               ...
               "% var, P=",format(fdist_paov$`Pr(>F)`[rownames(fdist_paov)=="genotype"], digits=2),"]",
                "\n arsen:genotype: ",
               format(fdist_paov$R2[rownames(fdist_paov)=="genotype:arsen"] * 100, digits=2, nsmall=1),
               "% var, P=",format(fdist_paov$`Pr(>F)`[rownames(fdist_paov)=="genotype:arsen"], digits=2),"]", sep = "")
#plot
p0 <- phyloseq::plot_ordination(phy_sub, fCAP_bray, color="genotype", shape="arsen", title =  "Fungi - CAP: Genotype:Arsen effect", axes=c(1,2))
p0$layers <- p0$layers[-1]
flines <- centroid_lines(phy_sub, fCAP_bray, c("genotype","arsen"))
p0 <- p0 + geom_segment(aes(xend=flines$Axis.1, yend=flines$Axis.2), x = flines$Axis.1.means, y = flines$Axis.2.means, alpha=0.3)
p0 <- p0 + geom_point(size=3)
#p0 <- p0 + geom_text(aes(label=fPHYSEQ@sam_data$fun_FLD))
p0 <- p0 + scale_color_manual(values=level_cols_genotype)
...
#print
#png("figs_for_paper/fig6.2.png",12000, 9000, res = 600)
fig_6.2
#dev.off()
...
**Conclusion:** There are differences in the bacterial and fungal communities due to the arsenic treatment, the genotypes and their interactions. We can explain about 4% of bacterial and 3% of fungal variety due to the arsen:genotype interaction effect.
