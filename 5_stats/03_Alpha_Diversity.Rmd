---
title: "Arsen - 03: Alpha Diversity"
author: "Jan Waelchli"
geometry: margin=2cm
output:
  pdf_document:
...
We investigate the effect on alpha diversity by the factors of genotype, arsen and the interaction between them. We model the alpha diversity against these factors in an aov-model and perform a F-Test.

```{r a-div overview, warning=F, echo=F, message=F, eval=T}

#BACTERIA
balpha_aov <- aov(asv_diversity ~ (genotype + arsen)^2, data=alpha_diversity_bDAT_summary) #^2 two way interactions
pander(summary(balpha_aov), caption = "Bacteria: F test")

#FUNGI
falpha_aov <- aov(asv_diversity ~ (genotype + arsen)^2, data=alpha_diversity_fDAT_summary)
...
```{r a-div Arsen, warning=F, echo=F, message=F, eval=T}

#BACTERIA
balpha_aov_tukey <- emmeans::emmeans(balpha_aov, c("genotype", "arsen"))
balpha_aov_tukey_letters <- multcomp::cld(balpha_aov_tukey, Letter="abcdefghi", alpha=0.05)
# means
balpha_means <- aggregate(alpha_diversity_bDAT_summary$asv_diversity,list(alpha_diversity_bDAT_summary$genotype, alpha_diversity_bDAT_summary$arsen), mean)
# table
balpha_aov_tukey_letters <-balpha_aov_tukey_letters[order(with(balpha_aov_tukey_letters, genotype)),]
balpha_aov_tukey_letters <-balpha_aov_tukey_letters[order(with(balpha_aov_tukey_letters, arsen)),]
balpha_means <- balpha_means[order(with(balpha_means, Group.1, Group.2)),]
balpha_table <- data.frame(genotype=balpha_means[,1], arsen=balpha_means[,2], Shanon=balpha_means[,3], letters=balpha_aov_tukey_letters[,8])
#pander(balpha_table, caption = "Bacteria: alpha diversity")

#FUNGI
falpha_aov_tukey <- emmeans::emmeans(falpha_aov, c("genotype", "arsen"))
falpha_aov_tukey_letters <- multcomp::cld(falpha_aov_tukey, Letter="abcdefghi", alpha=0.05)
# means
falpha_means <- aggregate(alpha_diversity_fDAT_summary$asv_diversity,list(alpha_diversity_fDAT_summary$genotype, alpha_diversity_fDAT_summary$arsen), mean)
# table
falpha_aov_tukey_letters <-falpha_aov_tukey_letters[order(with(falpha_aov_tukey_letters, genotype)),]
falpha_aov_tukey_letters <-falpha_aov_tukey_letters[order(with(falpha_aov_tukey_letters, arsen)),]
falpha_means <- falpha_means[order(with(falpha_means, Group.1, Group.2)),]
falpha_table <- data.frame(genotype=falpha_means[,1], arsen=falpha_means[,2], Shanon=falpha_means[,3], letters=falpha_aov_tukey_letters[,8])
#pander(falpha_table, caption = "Fungi: alpha diversity")

```

### Figure 5 \| Genotype*Arsen effect on alpha diversity
...
alpha_table <- alpha_table[order(alpha_table$taxa, alpha_table$arsen),] #make sure to have the same order as plot!
...
alpha_table$y <- c(
tapply(alpha_diversity_bDAT_summary$asv_diversity, list(alpha_diversity_bDAT_summary$genotype, alpha_diversity_bDAT_summary$arsen), max)+0.1,
tapply(alpha_diversity_fDAT_summary$asv_diversity, list(alpha_diversity_fDAT_summary$genotype, alpha_diversity_fDAT_summary$arsen), max)+0.1)

#plot
plot <- ggplot(alpha_diversity_DAT_summary, aes(y=asv_diversity, x=arsen, fill=genotype)) +
            ...
            labs(title = "Alpha diversity", subtitle = "alpha diversity ~ genotype * arsen") +
