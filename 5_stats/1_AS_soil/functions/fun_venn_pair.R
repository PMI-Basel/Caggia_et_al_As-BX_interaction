draw.pairwise.venn.shift <- function (area1, area2, cross.area, category = rep("", 2), euler.d = TRUE, 
                                scaled = TRUE, inverted = FALSE, shift=NULL, ext.text = TRUE, ext.percent = rep(0.05,3), 
                                ...
                                cat.pos = c(-50, 50), cat.dist = rep(0.025, 2), cat.cex = rep(1, 2), 
                                cat.col = rep("black", 2), cat.fontface = rep("plain",2), 
                                cat.fontfamily = rep("serif", 2), cat.just = rep(list(c(0.5,0.5)), 2), 
                                ...
                                ...) 
{
  ...
  if ((!scaled & !euler.d) | (!scaled & euler.d & !special.inclusion & 
    ...
    tmp <- textGrob(label = wrapLab(area1 - cross.area), 
                    x = 0.3 + shift, y = 0.5, gp = gpar(col = label.col[1], cex = cex[1], 
                                                        fontface = fontface[1], fontfamily = fontfamily[1]))
    grob.list <- gList(grob.list, tmp)
    tmp <- textGrob(label = wrapLab(area2 - cross.area), 
                    x = 0.7 + shift, y = 0.5, gp = gpar(col = label.col[3], cex = cex[3], 
                                                        fontface = fontface[3], fontfamily = fontfamily[3]))
    grob.list <- gList(grob.list, tmp)
    tmp <- textGrob(label = wrapLab(cross.area), x = 0.5 + shift, 
                    y = 0.5, gp = gpar(col = label.col[2], cex = cex[2], 
                                       fontface = fontface[2], fontfamily = fontfamily[2]))
    grob.list <- gList(grob.list, tmp)
    if (cat.default.pos == "outer") {
